...
//add headers for movingBox shape
...
#include <vector>
std::vector<hkpRigidBody*> masses; 
...
hkVector4 pos1 = hkVector4(0, 5, 0);	//fixed box position
hkVector4 pos2 = hkVector4(0, 3, 0);	//moving box position
...
void InitMemory() {
	...
	hkMemoryRouter* memoryRouter = hkMemoryInitUtil::initDefault( hkMallocAllocator::m_defaultMallocAllocator, hkMemorySystem::FrameInfo( 1000* 1024 ) );
...
void AddRigidBodies() {
	...
	//add the falling movingBox 
	{
		...
		for( j=0;j<v;j++) {
			for( i=0;i<u;i++) {
				if((j==0 && i==0) || (j==0 && i==numX)	)
					ci.m_motionType = hkpMotion::MOTION_FIXED;
				else
					ci.m_motionType = hkpMotion::MOTION_DYNAMIC;

				hkVector4f x( ((float(i)/(u-1)) *2-1)* hsize,size+1,((float(j)/(v-1) )* size));
				ci.m_position = x;
				hkpRigidBody* rigidBody = new hkpRigidBody(ci);
				masses.push_back(static_cast<hkpRigidBody*>(g_pWorld->addEntity(rigidBody)));
	...
	for (l1 = 0; l1 < v; l1++)	// v
		for (l2 = 0; l2 < (u - 1); l2++) {
			...
			hkpConstraintInstance* constraint = new hkpConstraintInstance(r0, r1, spring);
...
void DrawBox(hkpRigidBody* pRB, bool moving=false) { 
	float mat[16]={}; 
	...
	{ 
		...
		pRB->approxCurrentTransform( transform );
		m.set( transform ); 	 
		if(moving)
			pos2 = transform.getTranslation();
	...
	glBegin(GL_LINES);
		...
		g_pWorld->markForRead();
			for(size_t i=0;i<indices.size()-4;i+=4) { 
				...
				hkVector4f p1 = masses[i1]->getTransform().getTranslation();
				hkVector4f p2 = masses[i2]->getTransform().getTranslation();
				hkVector4f p3 = masses[i3]->getTransform().getTranslation();
				glVertex3f(p0.getComponent(0), p0.getComponent(1), p0.getComponent(2));
				glVertex3f(p1.getComponent(0), p1.getComponent(1), p1.getComponent(2));
				glVertex3f(p2.getComponent(0), p2.getComponent(1), p2.getComponent(2));
				glVertex3f(p3.getComponent(0), p3.getComponent(1), p3.getComponent(2));
			}
		g_pWorld->unmarkForRead();
	glEnd();
...
void main(int argc, char** argv) {
	...
	glutCreateWindow("GLUT Havok Demo - Cloth");
