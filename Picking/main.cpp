...
void OnRender() {
	...
	if(wasHit) {
		glBegin(GL_POINTS);
			glColor3f(1,0,0);
			glVertex3f(	intersectionPointWorld.getComponent(0),
						intersectionPointWorld.getComponent(1),
						intersectionPointWorld.getComponent(2));
		glEnd();
	}
	...
	SetOrthoForFont();		
		glColor3f(1,1,1);
		//Show the fps
		RenderSpacedBitmapString(20,20,0,GLUT_BITMAP_HELVETICA_12,buffer.str().c_str());
...
void MoveActor(int x, int y) {
	...
	g_pWorld->lock();		
		//hkTransform trans = mouseSphere->getTransform(); 
		//trans.setTranslation(pos); 
		//mouseSphere->setTransform(trans);
...
void OnKey(unsigned char key, int x, int y) {
	switch(key) {
		case 'r': {
			g_pWorld->lock();
				hkTransform trans = box->getTransform(); 
				trans.setIdentity();
				trans.setTranslation(hkVector4(0,10,0)); 
				box->setLinearVelocity(hkVector4(0,0,0));
				box->setAngularVelocity(hkVector4(0,0,0));
...
void main(int argc, char** argv) {
	...
	glutCreateWindow("GLUT Havok Demo - Picking");
